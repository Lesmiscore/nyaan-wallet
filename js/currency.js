...
module.exports = class {
  ...
  signTx(option) {
    ...
    if (!txb) {
      ...
      for (let i = 0; i < txb.inputs.length; i++) {
        txb.sign(
          i,
          accountNode.derive(path[0][0] | 0).derive(path[0][1] | 0).keyPair
        );
      }
      return txb.build();
    }
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      keyPair = accountNode.derive(path[i][0] | 0).derive(path[i][1] | 0)
        .keyPair;
  ...
  signMultisigTx(option) {
    ...
    for (let i = 0; i < txb.inputs.length; i++) {
      ...
      txb.sign(
        i,
        accountNode.derive(path[0] | 0).derive(path[1] | 0).keyPair,
        mSig.redeemScript
      );
    }
    if (option.complete) {
  ...
  signMessage(m, entropyCipher, password, path) {
    ...
    const node = this.lib.HDNode.fromSeedBuffer(
  ...
  getTxs(from, to) {
    ...
    return axios({
      ...
      data: qs.stringify({
        ...
        from,
        to,
        addrs: this.getReceiveAddr()
          .concat(this.getChangeAddr())
          .join(",")
      }),
      method: "POST"
    }).then(res => {
  ...
  callCP(method, params) {
    ...
    return axios
      .post(this.counterpartyEndpoint, {
        params,
        id: 0,
        jsonrpc: "2.0",
        method
